...
// Script represents one script record from the GPOS ScriptList.
type Script struct {
...
// Feature represents one feature record from the GPOS FeatureList.
type Feature struct {
	// Tag is the 4-character feature tag (e.g. "kern", "vkrn", "dist").
	Tag string
	// LookupListIndices lists lookup indices this feature activates.
	LookupListIndices []uint16
}

// Scripts returns the ScriptList parsed from a GPOS table.
//
// The caller can use this to filter kerning lookups by script and language.
...
// Features returns the FeatureList parsed from a GPOS table.
...
// the GPOS ScriptList/FeatureList/LookupList. This allows the caller to ask
...
func decodeScriptList(data []byte, offset int) ([]Script, error) {
	if offset+2 > len(data) {
		return nil, fmt.Errorf("GPOS ScriptList offset exceeds table")
	}
	scriptCount := int(readU16(data, offset))
	if offset+2+scriptCount*6 > len(data) {
		return nil, fmt.Errorf("GPOS ScriptList records exceed table")
	}
...
func decodeScript(data []byte, offset int) (Script, error) {
	if offset+4 > len(data) {
		return Script{}, fmt.Errorf("GPOS Script exceeds table")
	}
	script := Script{}
	...
	if offset+4+langSysCount*6 > len(data) {
		return Script{}, fmt.Errorf("GPOS LangSysRecords exceed table")
	}
...
func decodeLangSys(data []byte, offset int) (LangSys, error) {
	if offset+6 > len(data) {
		return LangSys{}, fmt.Errorf("GPOS LangSys exceeds table")
	}
	...
	if offset+6+featureIndexCount*2 > len(data) {
		return LangSys{}, fmt.Errorf("GPOS LangSys feature indices exceed table")
	}
...
func decodeFeatureList(data []byte, offset int) ([]Feature, error) {
	if offset+2 > len(data) {
		return nil, fmt.Errorf("GPOS FeatureList offset exceeds table")
	}
	featureCount := int(readU16(data, offset))
	if offset+2+featureCount*6 > len(data) {
		return nil, fmt.Errorf("GPOS FeatureList records exceed table")
	}
...
func decodeFeature(data []byte, offset int) (Feature, error) {
	if offset+4 > len(data) {
		return Feature{}, fmt.Errorf("GPOS Feature exceeds table")
	}
	lookupCount := int(readU16(data, offset+2))
	if offset+4+lookupCount*2 > len(data) {
		return Feature{}, fmt.Errorf("GPOS Feature lookup indices exceed table")
	}
